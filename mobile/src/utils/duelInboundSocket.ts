...
export function bindDuelSocketEvents(socket: Socket) {
  ...
  socket.on("queue_status", (p: { players_online?: number }) => store.dispatch(playersOnlineSet(p.players_online ?? 0)));
  socket.on("match_found", (p: { session_id?: string; opponent?: { username?: string; avatar_url?: string | null } }) => {
    const o = p.opponent;
    store.dispatch(matchFound({ sessionId: p.session_id ?? null, opponent: { username: String(o?.username ?? ""), avatarUrl: typeof o?.avatar_url === "string" ? o.avatar_url : null } }));
  });
  socket.on("round_start", (p: RoundStartWire) => {
    const q = p.question ?? {};
  ...
  socket.on("round_result", (p: { player_ids?: { player1?: string }; scores: { player1: number; player2: number }; correct_answer: string }) => {
    const p1 = p.player_ids?.player1;
    const first = (p1 && duelConnectionRefs.userId) ? p1 === duelConnectionRefs.userId : true;
    store.dispatch(roundResultReceived({ score: { me: first ? p.scores.player1 : p.scores.player2, opp: first ? p.scores.player2 : p.scores.player1 }, lastCorrectAnswer: p.correct_answer }));
  });
  socket.on("answer_feedback", (p: { isCorrect: boolean }) => {
    if (!p.isCorrect) store.dispatch(wrongAnswerIncremented());
  });
  socket.on("duel_end", (p: DuelEndWire) => {
    ...
    store.dispatch(duelEnded({ duelEnd: {
      won: !p.tied && (uid ? p.winner_user_id === uid : p.winner_user_id === socket.id),
