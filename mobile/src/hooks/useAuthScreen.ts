  ...
  const onSubmit = useCallback(async () => {
    ...
    try {
      if (isLogin) {
        ...
        const { registrationToken } = await authService.register(email, username, password, buildGuestLocalState(store.getState()));
        navigation.navigate("VerifyEmail", { email, registrationToken });
      }
